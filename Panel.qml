...
Panel {
  ...
  ipcTarget: "simonez.antigem"
  manageIpc: false

  property var anchorItem: null
  property var hostWidget: null
  readonly property var barIdentity: hostWidget || panelRoot
  readonly property var root: hostWidget || panelRoot
  ...
  function toggle() { panel.open ? close() : open() }
  function closeForPopoutSwitch() { panel.closeForPopoutSwitch() }

  Connections {
    target: panelRoot.hostWidget
    function onOpenedChanged() {
      if (panelRoot.hostWidget && panel.open !== panelRoot.hostWidget.opened) {
        panel.open = panelRoot.hostWidget.opened
      }
    }
  }
  ...
  KeyboardPanel {
    id: panel
    anchorItem: panelRoot.anchorItem || (panelRoot.hostWidget ? panelRoot.hostWidget.anchorButton : null)
    owner: panelRoot.barIdentity
    bar: panelRoot.bar
    open: panelRoot.hostWidget ? panelRoot.hostWidget.opened : false
    onOpenChanged: {
      if (panelRoot.hostWidget && panelRoot.hostWidget.opened !== open) {
        panelRoot.hostWidget.opened = open
      }
      if (open && panelRoot.hostWidget) {
        panelRoot.hostWidget.requestRefresh()
      }
    }
